...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for i in range(len(livingGhostPositionDistributions)):
            # argMax Return the position with the highest possibility
            ghostPosition = livingGhostPositionDistributions[i].argMax()
            distance = self.distancer.getDistance(pacmanPosition, ghostPosition)
            ghostDict[ghostPosition] = distance
        # get the key with minimum value, find the closest ghost position
        closestGhost = min(ghostDict, key=ghostDict.get)

        minDis = float('inf')
        bestAction = None
        # iterate legal actions
        for action in legal:
